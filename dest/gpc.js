...
var _static = gpcas.util.ArrayHelper;

_static.create2DArray = function(x,y){
    var a = [];
    for (var i=0; i<x; i++){
        a[i]= [];
    }
...
_static.valueEqual = function(obj1, obj2) {
    if (obj1==obj2) return true;
    if(equals(obj1, obj2)) return true;

    return false;
}
_static.sortPointsClockwise = function(vertices) {
...
var _static = gpcas.geometry.Clip;
...
_static.intersection = function(p1, p2, polyClass) {
 ...
 * @param polyClass The type of <code>Poly</code> to return
 */
_static.union = function(p1, p2, polyClass) {
 ...
 * @param polyClass The type of <code>Poly</code> to return
 */
_static.xor = function( p1, p2, polyClass) {
    if(polyClass==null || polyClass==undefined) {
        polyClass = "PolyDefault";
    }
 ...
 * @param polyClass The type of <code>Poly</code> to return
 */
_static.difference = function ( p1, p2, polyClass) {
    if(polyClass==null || polyClass==undefined) {
        polyClass = "PolyDefault";
    }
    return Clip.clip(OperationType.GPC_DIFF, p2, p1, polyClass );
}
_static.intersection = function( p1, p2) {
 ...
 */
_static.createNewPoly = function ( polyClass) {
    /* TODO :
     try
     {
 ...
 */
_static.clip = function ( op, subj, clip, polyClass) {
...
_static.EQ = function(a, b) {
    return (Math.abs(a - b) <= Clip.GPC_EPSILON);
}

_static.PREV_INDEX = function( i, n) {
    return ((i - 1+ n) % n);
}

_static.NEXT_INDEX = function(i, n) {
    return ((i + 1) % n);
}

_static.OPTIMAL = function ( p, i) {
    return (p.getY(Clip.PREV_INDEX (i, p.getNumPoints())) != p.getY(i)) ||
        (p.getY(Clip.NEXT_INDEX(i, p.getNumPoints())) != p.getY(i)) ;
}

_static.create_contour_bboxes = function (p)
{
    var box= [] ;
...
_static.minimax_test = function ( subj, clip, op){
...
_static.bound_list = function( lmt_table, y) {
    if( lmt_table.top_node == null )
    {
...
_static.insert_bound = function ( lmt_node, e) {
...
_static.add_edge_to_aet = function ( aet, edge) {
...
_static.add_to_sbtree = function ( sbte, y) {
...
_static.build_lmt = function( lmt_table, 
							sbte,
							p, 
...
_static.add_st_edge = function( st, it, edge, dy) {
    if (st == null)
    {
...
_static.add_intersection = function ( it_node,
    edge0,
    edge1,
    x,
    y) {
